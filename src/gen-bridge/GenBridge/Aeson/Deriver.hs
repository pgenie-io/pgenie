-- |
-- Shared Aeson TH deriver for the "GenBridge.Model.Input" data types,
-- pinning them to one JSON contract: kebab-case everywhere, sum types
-- encoded as a single-field object keyed by the constructor's discriminating
-- part (its type-name suffix stripped and spinal-cased).
module GenBridge.Aeson.Deriver
  ( derive,
  )
where
