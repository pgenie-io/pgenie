              ...
              inContext ("type:" <> type_.name) do
                case type_.elementTypeOid of
                  0 -> case type_.type_ of
                    ...
                    _ -> crashStitching ["Oops! Unexpected tag: ", TextBuilder.text type_.type_, ". Type name: ", TextBuilder.text type_.name]
...
warn reason = do
  location <- ask
  tell [Error (reverse location) reason]
...
crash reason = do
  location <- ask
  throwError (Error (reverse location) reason)
