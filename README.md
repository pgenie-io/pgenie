...
By default, each Docker-mode run starts a fresh PostgreSQL container and throws it away afterwards, which means paying the container's startup cost every time. Pass `--reuse` to keep the container running between invocations instead:
