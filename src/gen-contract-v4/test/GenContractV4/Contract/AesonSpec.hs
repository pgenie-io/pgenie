...
module GenContractV4.Contract.AesonSpec (spec) where

import Data.Aeson qualified as Aeson
import Data.Aeson.QQ.Simple (aesonQQ)
import GenContractV4.Contract qualified as Contract
import GenContractV4.Fixtures qualified as Fixtures
import Test.Hspec
import Prelude

spec :: Spec
spec = do
  describe "Contract model JSON contract" do
    ...
    it "encodes an all-nullary sum (Primitive) as a bare kebab-case string, splitting before digits" do
      Aeson.toJSON Contract.Int4Primitive `shouldBe` Aeson.String "int-4"
    ...
    it "encodes a non-nullary sum alternative (Scalar) as a single-field object keyed by the stripped, kebab-cased constructor" do
      Aeson.toJSON (Contract.PrimitiveScalar Contract.Int4Primitive)
        `shouldBe` [aesonQQ| { "primitive": "int-4" } |]

      Aeson.toJSON (Contract.CustomScalar exampleName)
    ...
    it "encodes nullary alternatives of a mixed sum (Result) as a single-field object with empty-array contents" do
      Aeson.toJSON Contract.VoidResult `shouldBe` [aesonQQ| { "void": [] } |]
      Aeson.toJSON Contract.RowsAffectedResult `shouldBe` [aesonQQ| { "rows-affected": [] } |]
    ...
    it "encodes a composite custom type definition tagged by stripping the type name" do
      Aeson.toJSON (Contract.CompositeCustomTypeDefinition [])
    ...
    it "encodes an enum custom type definition tagged by stripping the type name, kebab-casing enum variant fields" do
      Aeson.toJSON (Contract.EnumCustomTypeDefinition [Contract.EnumVariant {name = exampleName, pgName = "raw_enum"}])
        `shouldBe` Aeson.object
          [ ( "enum",
              Aeson.toJSON
                [ Aeson.object
    ...
    it "encodes a domain custom type definition tagged by stripping the type name" do
      Aeson.toJSON
        ( Contract.DomainCustomTypeDefinition
            Contract.Value {arraySettings = Nothing, scalar = Contract.PrimitiveScalar Contract.Int4Primitive}
        )
      ...
      Aeson.toJSON
        Contract.CustomType
          { name = exampleName,
            pgSchema = "public",
            pgName = "status",
            definition = Contract.EnumCustomTypeDefinition []
          }
        `shouldBe` Aeson.object
          [ ("name", exampleNameJson),
    ...
    it "encodes non-null ArraySettings with kebab-cased dimensionality/element-is-nullable fields" do
      Aeson.toJSON
        Contract.Value
          { arraySettings = Just Contract.ArraySettings {dimensionality = 2, elementIsNullable = True},
...
-- 'Contract.Query.srcPath' is now 'Text' (was 'Path'), so it no longer gets
