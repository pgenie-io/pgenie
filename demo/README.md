# pGenie Demo Project

This is a complete demonstration of pGenie in action, showing how it generates type-safe client libraries from SQL migrations and queries.

## Project Structure

```
demo/
├── project.pgn1.yaml      # Project configuration
├── freeze.pgn1.yaml       # Autogenerated file with frozen versions of codegens for reproducibility
├── migrations/            # Database schema migrations
│   ├── 1.sql              # Initial schema
│   └── 2.sql              # Schema updates
├── queries/               # Parameterized SQL queries
│   ├── insert_album.sql
│   ├── select_album_by_artist.sql
│   ├── select_genre_by_artist.sql
│   ├── update_album_released.sql
│   └── update_album_released_returning.sql
└── artifacts/             # Generated code artifacts
    └── hasql/             # Haskell hasql client library
```

## The Artifacts Directory

The `artifacts/` directory contains **generated code** that pGenie produces based on your migrations and queries. This directory is the output of running `pgn generate`.

### What's Inside

- **hasql/** — A complete Haskell library with type-safe query functions
  - Type-safe functions for each SQL query in `queries/`
  - Data types corresponding to your database schema
  - Properly typed parameters and return values
  - Ready to use with the [hasql](https://hackage.haskell.org/package/hasql) library

## Try It Yourself

To see pGenie in action:

1. **Delete the artifacts directory:**
   ```bash
   rm -rf artifacts
   ```

2. **Delete the freeze file:**
   ```bash
   rm freeze.pgn1.yaml
   ```

3. **Run the generator:**
   ```bash
   pgn generate
   ```

4. **Observe the results:**
   - The `artifacts/hasql/` directory is recreated
   - Type-safe Haskell code is generated from your SQL
   - The generated library is ready to use in your Haskell projects

## Important: First Run Notice

**The first time you run pGenie it will take 2-3 minutes** while it performs initial setup:

- **Docker image caching** — Downloads and caches the PostgreSQL Docker image
- **Code generator caching** — Downloads and caches the Dhall code for the configured generators

You may notice pGenie appears to "hang" on the "Loading" stage during this initial setup. This is normal!

**After the first run, subsequent executions will complete in just a few seconds.**

## Next Steps

- Modify a query in `queries/` and run `pgn generate` to see the artifacts update
- Add a new migration in `migrations/` to evolve your schema
- Explore the generated code in `artifacts/hasql/src/` to see the type-safe functions
