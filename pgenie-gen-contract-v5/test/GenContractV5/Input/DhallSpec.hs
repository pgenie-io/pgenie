...
module GenContractV5.Input.DhallSpec (spec) where
...
spec = do
  describe "Dhall/Haskell model compatibility" do
    it "Name record fields match" do
      checkRecordFields @Input.Name (contract <> ".Name")

    it "Version record fields match" do
      checkRecordFields @Input.Version (contract <> ".Version")

    it "Value record fields match" do
      checkRecordFields @Input.Value (contract <> ".Value")

    it "Member record fields match" do
      checkRecordFields @Input.Member (contract <> ".Member")

    it "EnumVariant record fields match" do
      checkRecordFields @Input.EnumVariant (contract <> ".EnumVariant")

    it "CustomType record fields match" do
      checkRecordFields @Input.CustomType (contract <> ".CustomType")

    it "CustomTypeRef record fields match" do
      checkRecordFields @Input.CustomTypeRef (contract <> ".CustomTypeRef")

    it "ResultRows record fields match" do
      checkRecordFields @Input.ResultRows (contract <> ".ResultRows")

    it "Var record fields match" do
      checkRecordFields @Input.Var (contract <> ".Var")

    it "Query record fields match" do
      checkRecordFields @Input.Query (contract <> ".Query")

    it "Project record fields match" do
      checkRecordFields @Input.Project (contract <> ".Project")

    it "Primitive union alternatives match" do
      checkUnionAlternatives @Input.Primitive (contract <> ".Primitive")

    it "Scalar union alternatives match" do
      checkUnionAlternatives @Input.Scalar (contract <> ".Scalar")

    it "CustomTypeDefinition union alternatives match" do
      checkUnionAlternatives @Input.CustomTypeDefinition (contract <> ".CustomTypeDefinition")

    it "ResultRowsCardinality union alternatives match" do
      checkUnionAlternatives @Input.ResultRowsCardinality (contract <> ".ResultRowsCardinality")

    it "Result union alternatives match" do
      checkUnionAlternatives @Input.Result (contract <> ".Result")

    it "QueryFragment union alternatives match" do
      checkUnionAlternatives @Input.QueryFragment (contract <> ".QueryFragment")

    it "ResultRowsCardinality Optional serializes/deserializes" do
      checkSerialization Input.OptionalResultRowsCardinality (contract <> ".ResultRowsCardinality.Optional")

    it "Result RowsAffected serializes/deserializes" do
      checkSerialization Input.RowsAffectedResult (contract <> ".Result.RowsAffected")
