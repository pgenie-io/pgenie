...
module GenContractV5.Contract.AesonSpec (spec) where
    ...
    it "encodes a domain custom type definition tagged by stripping the type name" do
      Aeson.toJSON
        ( Contract.DomainCustomTypeDefinition
            Contract.Value {dimensionality = 0, elementIsNullable = False, scalar = Contract.PrimitiveScalar Contract.Int4Primitive}
        )
